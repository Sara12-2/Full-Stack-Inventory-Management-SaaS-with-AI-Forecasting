...
const emptyForm: Partial<Supplier> = { name: "", email: "", phone: "" };
...
export default function SupplierForm({ open, supplier, onClose, onSave }: SupplierFormProps) {
  const [form, setForm] = useState<Partial<Supplier>>(emptyForm);

  useEffect(() => {
    setForm(supplier || emptyForm);
  }, [supplier, open]);
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-md rounded-2xl bg-card p-6 shadow-2xl dark:bg-card-dark">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-heading text-base font-semibold text-text-primary dark:text-text-primary-dark">{supplier ? "Edit Supplier" : "Add Supplier"}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button type="submit" className="rounded-lg bg-gradient-brand px-4 py-2.5 text-sm font-semibold text-white shadow-glow">
                  Save
                </button>
              </div>
            </form>
