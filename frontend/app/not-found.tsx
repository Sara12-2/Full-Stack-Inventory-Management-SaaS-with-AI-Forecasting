  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface p-6 dark:bg-surface-dark">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-8 text-center shadow-soft dark:border-border-dark dark:bg-card-dark dark:shadow-soft-dark">
        ...
        <h2 className="mt-4 font-heading text-lg font-bold text-text-primary dark:text-text-primary-dark">Page not found</h2>
        <p className="mt-1 text-sm text-text-secondary dark:text-text-secondary-dark">
          The page you're looking for doesn't exist or may have moved.
        </p>
        <Link
          href="/dashboard"
          className="mt-6 flex w-full items-center justify-center rounded-lg bg-gradient-brand py-2.5 text-sm font-semibold text-white shadow-glow transition-transform hover:scale-[1.02]"
        >
          Back to Dashboard
        </Link>
      </div>
